...
public static class CustomResponseWriter
{
    public static Task WriteResponse(HttpContext context, HealthReport healthReport)
    {
        ...
        using (var jsonWriter = new Utf8JsonWriter(memoryStream, options))
        {
            ...
            jsonWriter.WriteString("status", healthReport.Status.ToString().ToLower());
            jsonWriter.WriteString("version", Assembly.GetExecutingAssembly().GetName().Version?.ToString() ?? "0");
            ...
            foreach (var healthReportEntry in healthReport.Entries)
            {
                ...
                jsonWriter.WriteString("status",
                    healthReportEntry.Value.Status.ToString().ToLower());
